			...
			<section className="flex flex-col gap-12 p-4 w-full lg:w-1/2">
				...
				<form onSubmit={handleSignIn} className="flex flex-col gap-6 items-start">
					<div className="flex items-center w-full">
						...
						<input type="email" name="email" id="email" value={email} onChange={(e) => setEmail(e.target.value)} className="text-secondary-200 bg-secondary-700 w-3/4 p-1 px-2" />
					</div>
					<div className="flex items-center w-full">
						<label htmlFor="password" className="font-graduate w-1/4">
							Password
						</label>
						<input type="password" name="password" id="password" value={password} onChange={(e) => setPassword(e.target.value)} className="text-secondary-200 bg-secondary-700 w-3/4 p-1 px-2" />
					</div>
				...
				<p className="p-4 bg-secondary-800 text-secondary-300 clip-path-octagon-md italic">Your email will be kept private and will not be shared or used for any purpose other than to log into your account.</p>
			</section>
		</Main>
	)
}
