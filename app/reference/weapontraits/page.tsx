...
const page = () => {
	...
	const filteredTraits = sortedTraits.filter((trait) => trait.name.toLowerCase().includes(searchTerm.toLowerCase().trim()))
		...
		<main className="h-full bg-dataslate p-4 flex flex-col items-center">
			...
			<ul className="list-disc text-primary-950 ml-4">
				{filteredTraits.map((trait) => (
					<Link key={trait.name} href={`/reference/weapontraits/${trait.name.replaceAll(" ", "_")}`}>
						<li>{trait.name}</li>
					</Link>
				))}
			</ul>
