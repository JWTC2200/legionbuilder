...
const ListBuilderFormationSelector = ({ formation }: { formation: BUILDER_FORMATION }) => {
	...
	return (
		<div className="flex flex-wrap sm:gap-4 items-center justify-center">
			<select
				id={`formation_selector_${formation.ref_id}`}
				name={`formation_selector_${formation.ref_id}`}
				...
				{formationData.map((format) => (
					<option key={formation.ref_id + format.name} value={format.id} className="text-primary-950">
						{format.name}
					</option>
				))}
			</select>
