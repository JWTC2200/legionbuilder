...
const Detachment = ({ detachment }: properties) => {
	const { playList, orders, setOrders, viewMode } = playState()
	...
	return (
		<div
			className={`flex flex-col gap-1 sm:w-[450px] w-full justify-center items-start bg-secondary-800 p-2 px-4 sm:clip-path-octagon-md ${currentOrder?.activated ? "text-secondary-500" : ""}`}>
			<div className="w-full grid grid-cols-4">
				<div className="col-span-3">
					<div className="">{`${detachment.name} (${currentDetachmentSize(playList, detachment.slot_id)})`}</div>
