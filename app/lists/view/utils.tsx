...
const calcDetachmentBreakStrength = (unit: BUILDER_DETACHMENT_UNIT): number => {
	const base =
		unit.break_strength || unit.break_strength === 0 ? unit.break_strength * unit.base_size : unit.base_size
	...
	if (slot.selected_unit) {
		return (
			<li key={slot.slot_ref} className="relative">
				...
				<ReferencePopup detachmentId={slot.selected_unit.id}>{slot.selected_unit.name}</ReferencePopup>{" "}
...
export const upgradesHTML = (upgradeArray: BUILDER_DETACHMENT_UNIT_UPGRADES[], slot_ref: string) => {
	const upgrades = upgradeArray
		...
		.map((upgr) => <li key={slot_ref + upgr.name}>{upgr.text ? upgr.text : `${upgr.number} ${upgr.name}`}</li>)
	if (upgrades.length) {
