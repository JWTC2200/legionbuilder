...
const WeaponTraitBox = ({ trait }: { trait: WEAPON_TRAIT }) => {
  ...
  return (
    <div className="relative cursor-pointer underline decoration-dotted hover:text-cyan-700 active:text-cyan-700 relative group capitalize">
      {trait.name}
      {trait.value ? ` (${trait.value})` : null}
      {description?.tooltip ? (
        <article className="absolute hidden p-1 border-2 border-green-950 bg-green-50 group-hover:block bg-white rounded-lg w-max max-w-[200px] sm:max-w-[300px] -bottom-2 right-0 translate-y-full z-20 text-green-950 text-center normal-case">
