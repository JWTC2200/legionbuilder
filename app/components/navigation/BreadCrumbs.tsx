...
export function BreadCrumbs({ children }: { children: ReactNode }) {
  ...
  return (
    <header className="bg-zinc-900 p-4 py-2 text-xl font-graduate">
      ...
      {children.map((child: any, key: string) => (
        <span className="group" key={key}>
          {child}
          <span className="group-last:hidden px-2 text-zinc-500">&gt;</span>
        </span>
      ))}
    </header>
  );
...
export function Crumb({
  ...
  return !active ? (
    <Link href={href} className="text-lime-400 hover:text-lime-200">
      {children}
    </Link>
  ) : (
    <h1 className="inline text-zinc-300">{children}</h1>
  );
}
