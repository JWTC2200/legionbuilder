...
export default function NavBar() {
  ...
  return (
    <div className="w-full bg-lime-950 py-4 px-4 lg:px-8 flex lg:flex-col justify-between lg:justify-center gap-4 items-center text-center font-subrayada">
      <Logo/>
      <div className={'z-30 absolute top-0 right-0 bottom-0 left-0 lg:relative bg-stone-950/70 lg:bg-transparent backdrop-blur-md lg:backdrop-blur-none items-center justify-center text-2xl lg:text-base ' + (visible ? 'flex' : 'hidden lg:flex')} onClick={hide}>
            ...
            <NavItem path={"/builder"} icon={"clipboard"}>List builder</NavItem>
              {user ? (
                  <div className="hidden lg:flex flex-wrap gap-4 items-center justify-center">
                      <NavItem path={"/account/lists"} icon={"book"}>Lists</NavItem>
                      <NavItem path={"/account"} icon={"account"}>Account</NavItem>
