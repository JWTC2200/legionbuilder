...
export function ReferenceSelector() {
	...
	const [options] = useState(['units', 'weapons', 'detachments', 'formations'].map(option => {
	...
	return (
		<h1 className="relative inline-block text-secondary-300" ref={ref}>
			...
			<div
				className={`absolute z-50 bottom-o -left-2 ${open ? 'max-h-[130px]' : 'max-h-0'} bg-secondary-900 overflow-hidden transition-all`}>
				<ol className="p-2 pt-3 clip-path-halfagon-sm space-y-1">
					{options.map(option => {
						return (
							<li key={option.href} className="px-2">
								<Link href={option.href} className="text-primary-400 hover:text-primary-200"
								      onClick={toggleOpen}>{option.text}</Link>
							</li>
						)
					})}
				</ol>
