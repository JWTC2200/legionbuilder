    ...
    <header className="bg-secondary-900 p-4 py-2 text-lg font-graduate">
      ...
      {children.map((child: any, key: string) => (
        <span className="group" key={key}>
          {child}
          <span className="group-last:hidden px-2 text-secondary-500">
            &gt;
          </span>
        </span>
      ))}
...
export function ReferenceSelector() {
  ...
  return (
    <h1 className="relative inline-block text-secondary-300" ref={ref}>
      ...
      <div
        className={`absolute z-50 bottom-o -left-2 ${
          open ? "max-h-[130px]" : "max-h-0"
        } bg-secondary-900 overflow-hidden transition-all`}
      >
